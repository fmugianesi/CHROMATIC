# compute and save Genomic distance of detections in basepairs
...
for chrom in chroms:
    ...
    for ChIP in ChIPs:
        ...
        for p in np.unique(label_TP)[1:] :  # for each label = for each detected patch
            ...
            gd = np.round(np.abs(np.mean(x)-np.mean(y)))*res   # average genomic distance between the interacting loci
            GD_TP[i] = gd
            i += 1
        np.save(path + 'data/NPC/CHROMATIC/detections_analysis/genomic_distance_CHROMATIC_peaks_' + ChIP + '_' + chrom_reg + '.npy', GD_TP)
