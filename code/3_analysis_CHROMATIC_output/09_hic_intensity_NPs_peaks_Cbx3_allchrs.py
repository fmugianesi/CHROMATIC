...
for chrom in chroms:
    ...
    H = scipy.sparse.load_npz(path + 'data/NPC/HiC/npz/Hi-C_decay_' + chrom_reg + '_median.npz')    # load Hi-C matrix 
    ...
    for ChIP in ChIPs:
        ...
        for p in np.unique(label_TP)[1:] :  # for each label = for each detected patch
            ...
            gd = np.mean(H[x,y])   # average original Hi-C value in the pixels of the detection
            GD_TP[i] = gd
            i += 1
        np.save(path + 'data/NPC/CHROMATIC/detections_analysis/hic_intensity_CHROMATIC_peaks_' + ChIP + '_' + chrom_reg + '.npy', GD_TP)
