...
for chrom in chroms:
    ...
    for ChIP in ChIPs:
        ...
        H = scipy.sparse.load_npz(path + 'data/NPC/CHROMATIC/int_maps/' + ChIP + '_' + chrom_reg + '_CHROMATIC_int_map.npz')    # load CHROMATIC interaction map
        ...
        for p in np.unique(label_TP)[1:] :  # for each label = for each detected patch
            ...
            gd = np.mean(H[x,y])   # average CHROMATIC value in the pixels of the detection
            GD_TP[i] = gd
            i += 1
        np.save(path + 'data/NPC/CHROMATIC/detections_analysis/chromatic_score_CHROMATIC_peaks_' + ChIP + '_' + chrom_reg + '.npy', GD_TP)
